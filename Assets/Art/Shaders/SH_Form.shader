...
Shader "SH_Form"
{
	...
	SubShader
	{
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			c.rgb = ( half4( (( _ReflectionToggle )?( ( lerpResult81 * heightGradient112 ) ):( float3( 0,0,0 ) )) , 0.0 ) + ( ( (( _IndirectToggle )?( ( ( globalIllumination110 + ( albedo129 / 4.0 ) ) * half4( indirectDiffuse14 , 0.0 ) ) ):( float4( 0,0,0,0 ) )) + half4( (( _FresnelToggle )?( ( fresnelNode76 * ase_lightAtten * ase_lightColor.rgb ) ):( float3( 0,0,0 ) )) , 0.0 ) + half4( (( _LightingToggle )?( ( (( _CustomLightFalloffToggle )?( max( (dotResult7*_lightwrapscale + _lightwrapoffset) , 0.0 ) ):( max( dotResult7 , 0.0 ) )) * ( ase_lightAtten * ase_lightColor.rgb ) ) ):( float3( 0,0,0 ) )) , 0.0 ) ) * (( _AlbedoToggle )?( tex2D( _Albedo, uv_Albedo ) ):( float4( 1,1,1,0 ) )) ) ).rgb;
			c.a = 1;
			return c;
		}
...
/*ASEBEGIN
Version=18800
1920;0;1920;1019;1507.697;903.9792;1;True;False
...
ASEEND*/
//CHKSM=B80BAD221E80D6EA04AA390210A4C6C9EE6D0D01